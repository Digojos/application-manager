...
export function TvSessionsClient({ initialSessions }: TvSessionsClientProps) {
  ...
  const refreshSessions = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/basketball-sessions/active", { cache: "no-store" });
      if (!response.ok) {
            ...
            <article key={session.id} className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-lg">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Link
                  ...
                  className="inline-flex min-h-11 items-center justify-center rounded-xl bg-orange-400 px-5 py-2 text-sm font-semibold text-slate-950 transition-colors hover:bg-orange-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-200"
                >
                  Abrir visualização
                </Link>
              </div>
